...
void secondlvl() 
{
	...
	wchar_t save[10];
	f_in = fopen(name, "rb");
	fread(save, sizeof(wchar_t), 10, f_in);
	fclose(f_in);
	for (int i = 0; i < 10; i++)
	{
		save[i] = towupper(save[i]);
	}
	f_out = fopen(name, "wb");
	fwrite(save, sizeof(wchar_t), 10, f_out);
	fclose(f_out);
