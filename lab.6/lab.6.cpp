...
int main()
{
	...
	while (true)
	{
		...
		{
		case 1:
			cout << "Создать бинарный файл с информацией о предложенных к реализациителевизоры:\n- название фирмы;\n- размер по диагонали;\n- стоимость\nОпределить количество телевизоров фирмы «Samsung», размером более32 дюйма и распечатать сведения о них." << endl << endl;
			firstlvl();
			break;
		case 2:
			cout << "Добавить в Memo или в StringGrid некоторое количество символов русско-го алфавита и записать их в бинарный файл. Переписать бинарный файлтак, чтобы все символы были записаны заглавными буквами. Распеча-тать символы до и после изменения данных в файле." << endl << endl;
			secondlvl();
			break;
		case 3:
			cout << "Создать бинарный файл, компонентами которого является структура,возможностьая следующие поля:\n- фамилия и инициалы покупателя;\n- дата покупки;\n- общая стоимость приобретенного товара за первое полугодие;\n- общая стоимость приобретенного товара за второе полугодие;\n- начальный процент скидки на последующие приобретаемые товары;\nПереписать бинарный файл так, чтобы процентная скидка была быувеличено на 7%, если покупатель заплатил за предыдущий товар в первоми во втором полугодиях не менее 10000 грн." << endl << endl;
			thirdlvl();
			break;
		case 0:
			return(0);
